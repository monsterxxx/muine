...
angular.module('ps.muine.clubs.video', [])

.controller
('MuineClubsVideoCtrl', ['$scope', '$rootScope', 'Club', '$state', '$timeout',
function                ( $scope  ,  $rootScope ,  Club ,  $state ,  $timeout ){
  console.log('> ClubsVideoCtrl load');
  var doLog = true;

  //Data
  $scope.club = Club;

  $scope.slides = Club.video.slides;
  ...
  $scope.$on('$stateChangeStart', function (event, toState, toParams, fromState) {
    ...
    if (toState.name.indexOf('muine.clubs.club') === -1 &&  noPauseStates.indexOf(playerState) === -1) {
    ...
    if (fromState.name === 'muine.clubs.club.video' ) {
